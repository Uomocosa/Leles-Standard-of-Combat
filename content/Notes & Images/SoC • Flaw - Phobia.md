---
aliases:
  - Phobia
---
- ***Image***:<br>![[SoC • Flaw - Phobia (Image).png|222]]
- ***Info***: Flaw
- ***Cost***: 
- ***Description***:<br>Fear of a common thing. In its presence, no roll can succeed. 
- ***Effects***: 
- ***Limitations***: 
- ***Color***: green 
- _**Prompt** [ChatGPT](https://chatgpt.com/?model=auto)_: I need a 2D minimalistic fantasy illustration illustration, in a minimal graphic style with white and black color, and a clean white background. It represents the flaw ***Phobia***, with the following effect: Fear of a common thing. In its presence, no roll can succeed. Add the words "***Phobia***" under the image in a stylish font.
----
- ***Card***:![[SoC • Card - Phobia.png|550]]

----
- ***Card***:<br>![[SoC • Card - Phobia.png|550]]
