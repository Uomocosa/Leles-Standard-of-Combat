---
aliases:
  - Shallow Water
---
%%
- ***Image***:<br>![[Pasted image 20250531174004.png|222]]
- ***Alt***:<br>![[Pasted image 20250531173902.png|222]]
- ***Name***: Shallow Water 
- ***Nameplate***: Shallow Water 
- ***Type***: Zone 
- ***Description***:<br>_Where the biggest fish are actually just really ambitious guppies. Don't drown in the drama, darling, it's barely ankle-deep_
- ***Effects***: 
- ***Limitations***: 
- ***Color***: blue 
- ***Rarity***: common
- ***Image Filter***: fullart
- _**Prompt** [LMArena](https://beta.lmarena.ai/)_: I need a 2D epic fantasy illustration, in an epic graphic style. Deep Water seen from directly above.
%%

### Card

![[SoC • Card - Shallow Water.png|650]]
