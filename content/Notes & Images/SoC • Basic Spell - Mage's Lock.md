---
aliases:
  - Mage's Lock
---
%%
- ***OLD***:<br>![[SoC • Spell - Arcane Lock (Image).png|222]]
- ***Image***:<br>![[Pasted image 20250801160321.png|222]]
- ***Info***: [[SoC • Rule Card - Learning Basic Spells|Basic Spell]] 
- ***Cost***: 1 
- ***Description***:<br>A glyph used to magically seal a door or wall, it last **1 hour**. It also gains a magical shield giving it **5 additional [[SoC • Rules Chapter - HEALTH|Health]]**.
- ***Flavor***: Created from the Rune — [[SoC • Rune - Ward|Ward]]
- ***Limitations***:
- ***Color***: artifact 
- _**Prompt** [LMArena](https://beta.lmarena.ai/)_: I need a 2D epic fantasy illustration, in an epic graphic style with white and black color, and a clean white background. It represents a magical glyph on a closed door, and a magical aura all around the door. Add the words "Mage's Lock" under the image in a stylish font.
%%

![[SoC • Card - Mages Lock.png|650]]
